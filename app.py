...
def show_home():
    ...
    # 视频1
    with col1:
        video_path_1 = "assets/video-720.webm"
        video_id_1 = "home-video-1"
        
        # 使用HTML和JavaScript实现自动播放
        st.markdown(f"""
        <div style="display: flex; justify-content: center; margin: 10px 0;">
            <video id="{video_id_1}" autoplay loop muted playsinline 
                   style="max-width: 100%; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.2);">
                <source src="data:video/webm;base64,{base64.b64encode(open(video_path_1, 'rb').read()).decode()}" type="video/webm">
                您的浏览器不支持视频标签。
            </video>
        </div>
        <script>
            ...
            document.addEventListener('DOMContentLoaded', (event) => {{
                document.getElementById('{video_id_1}').play();
            }});
        </script>
        """, unsafe_allow_html=True)
    ...
    # 视频2
    with col2:
        video_path_2 = "assets/video2-720.webm"
        video_id_2 = "home-video-2"
        
        # 使用HTML和JavaScript实现自动播放
        st.markdown(f"""
        <div style="display: flex; justify-content: center; margin: 10px 0;">
            <video id="{video_id_2}" autoplay loop muted playsinline 
                   style="max-width: 100%; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.2);">
                <source src="data:video/webm;base64,{base64.b64encode(open(video_path_2, 'rb').read()).decode()}" type="video/webm">
                您的浏览器不支持视频标签。
            </video>
        </div>
        <script>
            // 确保视频自动播放
            document.addEventListener('DOMContentLoaded', (event) => {{
                document.getElementById('{video_id_2}').play();
            }});
        </script>
        """, unsafe_allow_html=True)
...
def show_paper_download():
    ...
    video_path = "assets/video-720.webm"
    ...
    st.markdown("---")
    st.markdown("Due to the limitation of the streamlit cloud, we cannot provide the remote desktop service. Please use the local version of the agent.Please refer to the [README](https://github.com/YajingSun-Group/oled_agent) for more details.")
